...
const B_BOTTOM = 118;                      // darüber endet der Text, darunter der Fuss
...
export function buildPdf({ title, subtitle, blocks, logo, footer, branding, style }) {
  ...
  const drawHeadBrief = (ersteSeite) => {
    ...
    if (ersteSeite) {
      if (title) {
        ...
        putT(title, RIGHT - tw, barY + 58, 18, true, 0.8, '1 1 1');
      }
      if (subtitle) {
  ...
  function drawTableBrief(b) {
    ...
    const layout = (cells) => {
      const per = cells.map((c, i) => wrapText(cellOf(c).text, cols[i].w - padL(i) - padR(i), size, cellOf(c).bold));
      return { per, zeilen: Math.max(1, ...per.map((l) => l.length)) };
    };
    ...
    for (const r of (b.rows || [])) {
      ...
      const hoehe = L.zeilen * lh + 2 * padY;
      if (y - hoehe < BOTTOM) { newPage(); if (hatKopf) kopf(); }
      const top = y;
      let x = LEFT;
      cells.forEach((raw, i) => {
        const c = cellOf(raw);
        ...
        L.per[i].forEach((ln, k) => {
          if (!ln) return;
          const by = top - padY - (k + 1) * lh + lh * 0.3;
          const tw = textWidth(ln, size, c.bold);
          const tx = align === 'right' ? x + cols[i].w - padR(i) - tw
            : align === 'center' ? x + (cols[i].w - tw) / 2
              : x + padL(i);
          put(ln, tx, by, size, c.bold);
        });
        x += cols[i].w;
      });
      y = top - hoehe;
  ...
  // ── Fusszeile im Stil 'brief': Logo mittig, darunter Fusszeile aus
  //    gs_branding, darunter die Seitenzählung. Auf jeder Seite gleich hoch. ──
  if (brief) {
    pages.forEach((pg, i) => {
      pg.push(`${HAIR} RG 0.5 w ${LEFT} 100 m ${RIGHT} 100 l S`);
      if (logoImg) {
        const lh = 30, lw = lh * (logoImg.info.width / logoImg.info.height);
        pg.push(`q ${lw.toFixed(2)} 0 0 ${lh} ${((PAGE_W - lw) / 2).toFixed(2)} 62 cm /${logoImg.name} Do Q`);
      }
      const mitte = (txt, size, baseline) => {
        const w = textWidth(txt, size, false);
        pg.push(`q ${LABELCOL} rg BT /F2 ${size} Tf ${((PAGE_W - w) / 2).toFixed(2)} ${baseline} Td (${esc(txt)}) Tj ET Q`);
      };
      if (marke.fusszeile) mitte(marke.fusszeile, 7.5, 46);
      mitte(`Seite ${i + 1} von ${pages.length}`, 7.5, 32);
    });
  }
