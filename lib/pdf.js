...
//   • PNG-Bilder über pngToGray(): entpacken, Filter zurücknehmen, auf Weiss
//     flachlegen, als 8-Bit-Graustufen (/FlateDecode) einbetten. Gebraucht für
//     die Unterschriften im Wochenrapport, die als PNG mit Alpha vorliegen.
//     Nutzt node:zlib aus der Standardbibliothek — weiterhin keine Abhängigkeit.
...
import zlib from 'node:zlib';   // Node-Standardbibliothek, keine Abhaengigkeit
...
// ═══════════════════════════════════════════════════════════════════════════
// PNG — einlesen und als Graustufenbild aufbereiten
// ═══════════════════════════════════════════════════════════════════════════
// Warum ueberhaupt: Unterschriften werden als PNG mit Alphakanal gespeichert
// (api/cockpit.js schreibt unterschriften/<id>/technik.png). /DCTDecode kann
// damit nichts anfangen, und ein PNG mit Alpha laesst sich nicht roh in ein
// PDF durchreichen — die Alphamaske muesste als eigenes SMask-Objekt daneben.
//
// Statt dessen der einfache, fuer diesen Zweck richtige Weg: entpacken,
// PNG-Filter zuruecknehmen, auf WEISS flachlegen und als 8-Bit-Graustufen
// wieder komprimieren. Eine Unterschrift ist Tinte auf Papier, kein Farbbild;
// Graustufen halbieren nebenbei die Dateigroesse.
//
// zlib kommt aus der Node-Standardbibliothek — keine neue Abhaengigkeit.
// Nicht unterstuetzt: interlaced (Adam7) und Bittiefen ausser 8. Beides
// liefert null, der Aufrufer zeigt dann "Bild nicht darstellbar" statt einer
// stillen Leerflaeche.
export function pngToGray(buf) {
  ...
  let p = 8, w = 0, h = 0, bitDepth = 0, colorType = -1, interlace = 0;
  let palette = null, trns = null;
  const idat = [];
  while (p + 8 <= buf.length) {
    const len = buf.readUInt32BE(p);
    const typ = buf.toString('latin1', p + 4, p + 8);
    const dat = buf.subarray(p + 8, p + 8 + len);
    if (typ === 'IHDR') {
      w = dat.readUInt32BE(0); h = dat.readUInt32BE(4);
      bitDepth = dat[8]; colorType = dat[9]; interlace = dat[12];
    } else if (typ === 'PLTE') palette = Buffer.from(dat);
    else if (typ === 'tRNS') trns = Buffer.from(dat);
    else if (typ === 'IDAT') idat.push(Buffer.from(dat));
    else if (typ === 'IEND') break;
    p += 12 + len;                                   // len + typ + daten + crc
  }
  if (!w || !h || bitDepth !== 8 || interlace !== 0 || !idat.length) return null;
  const KANAELE = { 0: 1, 2: 3, 3: 1, 4: 2, 6: 4 };
  const kan = KANAELE[colorType];
  if (!kan) return null;
  if (colorType === 3 && (!palette || palette.length < 3)) return null;

  let roh;
  try { roh = zlib.inflateSync(Buffer.concat(idat)); } catch (_) { return null; }
  const zeile = w * kan;
  if (roh.length < (zeile + 1) * h) return null;

  // PNG-Filter zuruecknehmen (Filtertypen 0-4, je Zeile ein Byte davor).
  const bpp = kan;                                    // bitDepth 8 -> Bytes je Pixel
  const bild = Buffer.alloc(zeile * h);
  for (let y = 0; y < h; y++) {
    const ft = roh[y * (zeile + 1)];
    const src = roh.subarray(y * (zeile + 1) + 1, y * (zeile + 1) + 1 + zeile);
    const ziel = bild.subarray(y * zeile, (y + 1) * zeile);
  ...
  // Auf Weiss flachlegen und in Graustufen umrechnen.
  const grau = Buffer.alloc(w * h);
  for (let i = 0, n = w * h; i < n; i++) {
    let r, g, b, alpha = 255;
    if (colorType === 0) { r = g = b = bild[i]; }
    else if (colorType === 4) { r = g = b = bild[i * 2]; alpha = bild[i * 2 + 1]; }
    else if (colorType === 2) { r = bild[i * 3]; g = bild[i * 3 + 1]; b = bild[i * 3 + 2]; }
    else if (colorType === 6) { r = bild[i * 4]; g = bild[i * 4 + 1]; b = bild[i * 4 + 2]; alpha = bild[i * 4 + 3]; }
    else {                                            // Palette
      const idx = bild[i] * 3;
      r = palette[idx]; g = palette[idx + 1]; b = palette[idx + 2];
      if (trns && bild[i] < trns.length) alpha = trns[bild[i]];
    }
    const lum = 0.299 * r + 0.587 * g + 0.114 * b;
    grau[i] = Math.round(lum * (alpha / 255) + 255 * (1 - alpha / 255));
  }
  let data;
  try { data = zlib.deflateSync(grau, { level: 9 }); } catch (_) { return null; }
  return { width: w, height: h, components: 1, filter: '/FlateDecode', data };
}
...
export function buildPdf({ title, subtitle, blocks, logo, footer, branding, style, balance }) {
  ...
  const images = [];                       // [{buf, info, stream, filter}]
  const imgRef = (buf) => {
    if (!Buffer.isBuffer(buf)) return null;
    // Dedup vor dem Dekodieren: dieselbe Unterschrift steht oft mehrfach im
    // Dokument, und pngToGray ist der teuerste Schritt hier.
    const found = images.findIndex((x) => x.buf.equals(buf));
    if (found >= 0) return { name: `Im${found}`, info: images[found].info };

    let info = jpegInfo(buf);
    let stream = buf, filter = '/DCTDecode';
    if (info) {
      if (info.progressive || !(info.width > 0) || !(info.height > 0)) return null;
      if (info.components !== 1 && info.components !== 3 && info.components !== 4) return null;
    } else {
      // Kein JPEG → PNG versuchen (Unterschriften liegen als PNG mit Alpha vor).
      const png = pngToGray(buf);
      if (!png) return null;
      info = { width: png.width, height: png.height, components: 1, progressive: false };
      stream = png.data; filter = png.filter;
    }
    images.push({ buf, info, stream, filter });
  ...
  let seitenEnde = [];
  // ── Verweise (Phase 8) ───────────────────────────────────────────────────
  // Eine Liste je Seite: { rect:[x1,y1,x2,y2], url }. PDF kennt dafuer
  // /Annots am Seitenobjekt; die Rechtecke stehen in PDF-Koordinaten
  // (Ursprung unten links), genau wie alles andere hier.
  //
  // WICHTIG: der Satz laeuft beim Seitenausgleich MEHRFACH (durchgang). Die
  // Liste muss deshalb bei jedem Durchgang mitgeleert werden — sonst
  // sammelten sich die Rechtecke aller Versuche an und der letzte Stand
  // truege Verweise an Stellen, an denen nichts mehr steht.
  let linksJeSeite = [];
  const linkSetzen = (rect, url) => {
    if (!url) return;
    const i = pages.length - 1;
    if (i < 0) return;
    (linksJeSeite[i] || (linksJeSeite[i] = [])).push({ rect, url: String(url) });
  };
  ...
  function drawImages(b) {
    ...
    // Phase 8 — je Bild ein optionaler Verweis (gleiche Reihenfolge wie
    // b.images). Ohne Eintrag bleibt das Bild ein Bild.
    ...
    for (let i = 0; i < list.length; i += perRow) {
      ...
      drawn.forEach((d, k) => {
        ...
        // Das ganze Bild ist die Klickflaeche — auf Papier ist es ein
        // Standbild, im PDF fuehrt es zum Video.
        linkSetzen([x, top - d.h, x + d.w, top], links[i + k]);
  ...
  for (const im of images) {
    ...
    // Betrifft nur DCTDecode; aus pngToGray kommt immer 1 Komponente.
    const dec = (im.filter === '/DCTDecode' && im.info.components === 4) ? '/Decode[1 0 1 0 1 0 1 0]' : '';
    const roh = im.stream || im.buf;
    objs.push({
      head: `<</Type/XObject/Subtype/Image/Width ${im.info.width}/Height ${im.info.height}`
        + `/ColorSpace ${cs}${dec}/BitsPerComponent 8/Filter${im.filter || '/DCTDecode'}/Length ${roh.length}>>`,
      stream: roh,
    });
  }
  // Verweis-Objekte kommen NACH allen Seiten. Ihre Objektnummern lassen sich
  // vorher ausrechnen, weil hier nichts mehr dazwischenkommt: 4 feste Objekte
  // + Bilder + je Seite zwei (Seite und Inhalt).
  const annotBase = 4 + images.length + pages.length * 2 + 1;
  let annotNr = annotBase;
  const annotObjs = [];
  for (let i = 0; i < pages.length; i++) {
    const pg = pages[i];
    const content = pg.join('\n');
    const meine = linksJeSeite[i] || [];
    let annots = '';
    if (meine.length) {
      const refs = meine.map(() => `${annotNr++} 0 R`).join(' ');
      annots = `/Annots[${refs}]`;
      for (const l of meine) {
        const [x1, y1, x2, y2] = l.rect;
        // /Border[0 0 0]: kein Rahmen. Ein blauer Kasten um jedes Standbild
        // waere Berichtsdesign, und darum geht es hier nicht.
        annotObjs.push({
          head: `<</Type/Annot/Subtype/Link/Rect[${x1.toFixed(2)} ${y1.toFixed(2)} ${x2.toFixed(2)} ${y2.toFixed(2)}]`
            + `/Border[0 0 0]/A<</S/URI/URI(${esc(l.url)})>>>>`,
        });
      }
    }
    objs.push({ head: `<</Type/Page/Parent 2 0 R/MediaBox[0 0 ${PAGE_W} ${PAGE_H}]/Resources ${resources}${annots}/Contents ${objs.length + 2} 0 R>>` });
    objs.push({ head: `<</Length ${Buffer.byteLength(content, 'latin1')}>>`, stream: Buffer.from(content, 'latin1') });
  }
  for (const a of annotObjs) objs.push(a);
