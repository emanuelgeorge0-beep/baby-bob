...
export function buildPdf({ title, subtitle, blocks, logo, footer, branding, style, balance }) {
  ...
  // Eine letzte Seite, die nur eine Tabellenzeile und drei Zeilen Text trägt,
  ...
  // hinten, bis die letzte Seite mindestens ein Drittel füllt. Sobald das eine
  // ZUSÄTZLICHE Seite kosten würde, wird der letzte gute Stand wiederhergestellt
  // — lieber eine etwas leere Seite als eine Seite mehr.
  durchgang(0);
  if (brief && balance && pages.length > 1) {
    const folgeTop = PAGE_H - BAR2_H - 18;     // Textbeginn einer Folgeseite
    const fuellgrad = () => (folgeTop - y) / (folgeTop - bottomEff);
    const seitenZiel = pages.length;
    let extra = 0;
    while (fuellgrad() < 0.34 && extra < 260) {
      const probe = extra + 18;
      durchgang(probe);
      if (pages.length !== seitenZiel) { durchgang(extra); break; }
      extra = probe;
    }
  }
  ...
  if (brief) {
    pages.forEach((pg, i) => {
      ...
      mitte(`Seite ${i + 1} von ${pages.length}`, 7.5, 42);
    });
  }
