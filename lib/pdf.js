...
export function buildBlockadenReportPdf({ kw, jahr, blockaden, projektName }) {
  ...
  if (!list.length) {
    ...
    // Nach Dringlichkeit sortiert (CRITICAL zuerst). Einseitiges PDF → auf 10 Details begrenzen.
    const rank = { CRITICAL: 0, HIGH: 1, MEDIUM: 2, LOW: 3 };
    const sorted = list.slice().sort((a, b) => (rank[a.urgency] ?? 9) - (rank[b.urgency] ?? 9));
    const MAX = 10;
    sorted.slice(0, MAX).forEach((b, i) => {
      ...
      blocks.push({ t: 'text', text: (b.beschreibung || '–').slice(0, 180) });
      if (b.resolution) blocks.push({ t: 'kv', label: 'Auflösung', value: b.resolution.slice(0, 160) });
      blocks.push({ t: 'sp', size: 4 });
    });
    if (sorted.length > MAX) blocks.push({ t: 'text', text: `… und ${sorted.length - MAX} weitere Blockaden (siehe Übersicht in der App).` });
  }
