...
import { abwesenheitLabel, trenneStunden, abwesenheitBloecke } from './abwesenheit.js';
...
// Die Beschriftung der Abwesenheitskürzel kommt aus lib/abwesenheit.js, damit
// Auswahlfeld, Prüfung und PDF dieselbe Liste sehen (abwesenheitLabel()).
// Hier stand bis zur Feldrunde eine EIGENE Liste, in der 'G' „Gleitzeit" hiess,
// während das Auswahlfeld in app.html dieselbe Taste „Gesetzl. Feiertag" nannte.
// Der Techniker wählt die Beschriftung, die er sieht — also gilt seine.
...
// Stunden werden je Tag addiert, Spesen fallen je Tag EINMAL an — unabhaengig
// von der Zahl der Baustellen. Genommen wird der hoechste Wert des Tages, nicht
// die Summe (das ist der Fehler) und nicht die erste Zeile (keine zugesicherte
// Reihenfolge). Spiegelt spesenJeTag() in api/cockpit.js.
...
// ═══════════════════════════════════════════════════════════════════════════
// Fotos ohne Tagesbindung — welche Woche traegt sie?
// ═══════════════════════════════════════════════════════════════════════════
// Ein Foto ohne Tagesbindung hat KEIN Datum. Bis hierher entschied ersatzweise
// der Hochladezeitpunkt (created_at) im Wochenbericht, waehrend die
// Fotodokumentation gar nicht einschraenkte — dieselben zehn Bilder standen
// deshalb in KW 29, 30 UND 31, und die 30 Bilder vom 24.08. fielen aus KW 34
// heraus, obwohl dort gearbeitet wurde.
//
// Ab hier entscheidet die LEISTUNG statt der Hochladezeit:
//   a) Hat das Projekt in genau einer Woche Stunden, gehoert das Foto in den
//      Bericht dieser Woche.
//   b) Hat es in mehreren Wochen Stunden, erscheint es in KEINER — statt in
//      allen. Die Zahl wird gemeldet, nicht verschwiegen.
//   c) Ein gesetzter Tag hat immer Vorrang: solche Fotos kommen hier gar nicht
//      an, sie haengen ueber tagesrapport_id an ihrem Tag.
//
// Tageszeilen ohne Stunden zaehlen nicht: eine Zeile mit 0.00 h ist keine
// Woche, in der auf dem Projekt gearbeitet wurde.
const wochenSchluessel = (jahr, woche) => `${jahr}-${String(woche).padStart(2, '0')}`;

// Schluessel '2026-29' → 'KW 29/2026' fuer Dokument und Oberflaeche.
export function wochenText(schluessel) {
  return (schluessel || []).map((k) => {
    const [j, w] = String(k).split('-');
    return `KW ${Number(w)}/${j}`;
  }).join(', ');
}

// Je Projekt die ISO-Wochen mit Stunden. Rueckgabe: Map(projekt_id → Set('2026-29')).
// Bewusst OHNE sbSoft: ein Lesefehler darf nicht als "keine Woche" durchgehen —
// dann verschwaenden Fotos still aus jedem Bericht. Fehler schlaegt durch.
export async function wochenMitStunden(projektIds) {
  const ids = [...new Set((projektIds || []).filter(Boolean))];
  const map = new Map(ids.map((id) => [id, new Set()]));
  if (!ids.length) return map;
  const rows = await sbGet(
    `gs_tagesrapporte?projekt_id=in.(${ids.join(',')})&gesamtstunden=gt.0`
    + '&select=projekt_id,datum,gesamtstunden&order=datum.asc',
  );
  for (const z of rows || []) {
    if (!z.projekt_id || !z.datum || num(z.gesamtstunden) <= 0) continue;
    const { jahr, woche } = isoWocheVonDatum(z.datum);
    if (!jahr || !woche) continue;
    const set = map.get(z.projekt_id);
    if (set) set.add(wochenSchluessel(jahr, woche));
  }
  return map;
}

// Urteil fuer die Fotos OHNE Tag eines Projekts in einer bestimmten Woche.
//   zeigen === true   → sie gehoeren in genau diesen Bericht
//   'andere_woche'    → sie gehoeren in den Bericht einer anderen Woche
//   'mehrdeutig'      → mehrere Wochen mit Stunden, sie gehoeren in keine
//   'keine_stunden'   → auf dem Projekt sind ueberhaupt keine Stunden erfasst
export function fotoWochenurteil({ wochen, jahr, woche }) {
  const liste = [...(wochen || [])].sort();
  const hier = wochenSchluessel(Number(jahr), Number(woche));
  if (liste.length === 1 && liste[0] === hier) return { zeigen: true, grund: 'eindeutig', wochen: liste };
  if (liste.length === 1) return { zeigen: false, grund: 'andere_woche', wochen: liste };
  if (!liste.length) return { zeigen: false, grund: 'keine_stunden', wochen: liste };
  return { zeigen: false, grund: 'mehrdeutig', wochen: liste };
}

// Ein Satz, der die gemeldete Zahl erklaert. Steht so im PDF und in der
// Oberflaeche — dieselbe Wahrheit an beiden Orten.
export function ohneZuordnungText(oz) {
  if (!oz || !oz.anzahl) return '';
  const w = wochenText(oz.wochen || []);
  if (oz.grund === 'andere_woche') {
    return `${oz.anzahl} Foto(s) ohne Tageszuordnung gehören zu ${w} und stehen im Bericht jener Woche.`;
  }
  if (oz.grund === 'keine_stunden') {
    return `${oz.anzahl} Foto(s) ohne Tageszuordnung: auf diesem Projekt sind keine Stunden erfasst, deshalb lassen sie sich keiner Woche zuordnen. Zuordnung im Cockpit über die Projektgalerie.`;
  }
  return `${oz.anzahl} Foto(s) ohne Tageszuordnung: das Projekt hat Stunden in ${w}. Sie erscheinen deshalb in keiner Woche, bis ihnen im Cockpit ein Tag zugewiesen wird.`;
}
...
export async function sammleWochendaten({ quelle = 'projekt', projektId = null, serviceAuftragId = null, jahr, woche }) {
  ...
  // Kunde: Firma fuer den Kopf, E-Mail als Empfaenger-Rueckfallebene. Dieselbe
  // Abfrage, ein Feld mehr — kein zusaetzlicher Request.
  let kunde = null, kundeEmail = null;
  if (p.kunde_id) {
    const kd = await sbSoft(`gs_kunden?id=eq.${p.kunde_id}&select=firma,email&limit=1`, []);
    kunde = (kd[0] || {}).firma || null;
    kundeEmail = (kd[0] || {}).email || null;
  }

  // Partnerprofil: letzte Empfaenger-Rueckfallebene. Live steht die Adresse des
  // Bauleiters oft NUR hier (gs_projekte.ansprech_email ist bei fast allen
  // Projekten leer), waehrend gs_partner_profil.email gepflegt ist. sbSoft, weil
  // ein Projekt ohne Partner der Normalfall ist und den Kopf nicht scheitern
  // lassen darf — gleiches Muster wie die kuerzel-Abfrage oben.
  let partnerEmail = null;
  if (p.partner_user_id) {
    const pp = await sbSoft(`gs_partner_profil?partner_user_id=eq.${p.partner_user_id}&select=email&limit=1`, []);
  ...
  const kopf = {
    ...
    // Rueckfallebenen fuer den Empfaenger (siehe empfaengerFuer). Rein
    // informativ im Kopf — sie ersetzen ansprech_email NICHT, sie folgen ihr.
  ...
  if (zeilenIds.length) {
    const m = await sbSoft(
      ...
      // thumbnail_path: die kleine, fuer die Abbildung gemachte Fassung
      // (Phase 9). Fehlt sie, nimmt fotoQuelle() das Original.
      + '&select=id,tagesrapport_id,bucket,path,thumbnail_path,dateiname,stockwerk,wohnung,raum,bauabschnitt,notiz,created_at'
    ...
    fotos = (m || []).map((x) => ({
      ...
      path: x.path,
      thumbnail_path: x.thumbnail_path || null,
  ...
  // ── 5a-2. VIDEOS (Phase 8) ───────────────────────────────────────────────
  // Gleiche Zuordnung wie bei den Fotos: ueber tagesrapport_id, nie ueber
  // created_at. Ins PDF kommt nicht das Video, sondern sein Standbild — mit
  // einem Verweis auf /v/<token>. Ein Video im PDF gibt es nicht; ein
  // Standbild ohne Weg zum Video waere eine Sackgasse.
  let videos = [];
  if (zeilenIds.length) {
    const mv = await sbSoft(
      `gs_projekt_medien?tagesrapport_id=in.(${zeilenIds.join(',')})&medientyp=eq.video`
      + '&select=id,tagesrapport_id,bucket,path,thumbnail_path,dateiname,dauer_sekunden,'
      + 'stockwerk,wohnung,raum,bauabschnitt,notiz,created_at&order=created_at.asc',
      [],
    );
    const datumVon2 = {};
    for (const z of zeilen) datumVon2[z.id] = z.datum;
    videos = (mv || []).map((x) => ({
      id: x.id,
      tagesrapport_id: x.tagesrapport_id || null,
      bucket: x.bucket || MEDIEN_BUCKET,
      ...
      thumbnail_path: x.thumbnail_path || null,
      dateiname: x.dateiname || null,
      dauer_sekunden: x.dauer_sekunden || null,
      datum: datumVon2[x.tagesrapport_id] || null,
      ort: [x.stockwerk, x.wohnung, x.raum, x.bauabschnitt].filter(Boolean).join(' · ') || null,
      notiz: x.notiz || null,
    }));
  }
  ...
  //
  // WAS HIER FRUEHER STAND: ein Fenster auf created_at, den HOCHLADE-Zeitpunkt.
  // Das war nachweislich falsch — 30 Bilder wurden am 24.08. hochgeladen, die
  // dokumentierte KW 34 endet am 23.08., der Bericht meldete null Fotos.
  // Massgeblich ist jetzt die Leistung: die Wochen, in denen dieses Projekt
  // Stunden hat (fotoWochenurteil). Passt diese Woche nicht, erscheint kein
  // Bild — die Zahl wird aber gemeldet, siehe fotos_ohne_zuordnung.
  const wochenDesProjekts = (await wochenMitStunden([projektId])).get(projektId) || new Set();
  const urteil = fotoWochenurteil({ wochen: wochenDesProjekts, jahr: j, woche: w });
  // sbSoft mit null statt []: ein Lesefehler sieht sonst aus wie "keine Fotos".
  // Er wird benannt, nicht verschwiegen.
  const medienRoh = await sbSoft(
    `gs_projekt_medien?projekt_id=eq.${projektId}&tagesrapport_id=is.null&medientyp=eq.foto`
    + '&select=id,bucket,path,thumbnail_path,dateiname,stockwerk,wohnung,raum,bauabschnitt,notiz,created_at'
    + '&order=created_at.asc',
    null,
  );
  if (medienRoh === null) hinweise.push('Die Fotos ohne Tageszuordnung konnten nicht gelesen werden — dieser Abschnitt bleibt leer.');
  const ohneTagAlle = (medienRoh || []).map((x) => ({
    id: x.id,
    bucket: x.bucket || MEDIEN_BUCKET,
    path: x.path,
    thumbnail_path: x.thumbnail_path || null,
  ...
  const fotosOhneTag = urteil.zeigen ? ohneTagAlle : [];
  // Nicht gezeigt heisst NICHT verschwiegen: was hier nicht erscheint, wird
  // gezaehlt und im Dokument benannt.
  const fotosOhneZuordnung = {
    anzahl: urteil.zeigen ? 0 : ohneTagAlle.length,
    grund: urteil.zeigen ? null : urteil.grund,
    wochen: urteil.wochen,
  };

  // ── 5c. Fuehrendes Projekt je Kalendertag ────────────────────────────────
  // Die Tagespauschale faellt EINMAL je Tag an. Dieser Bericht sieht aber nur
  // die Zeilen SEINES Projekts — er kann von sich aus nicht wissen, ob der Tag
  // noch auf anderen Baustellen lief. Ohne diese Zusatzabfrage wiesen vier
  // Projektberichte desselben Tages die Pauschale viermal aus (KW 34: 4 x 30
  // statt 1 x 30 fuer den 17./18.08.).
  //
  // Zugerechnet wird dem Projekt mit den MEISTEN Stunden an diesem Tag; bei
  // Gleichstand der kleinsten Projektnummer, damit das Ergebnis stabil ist und
  // nicht von der Zeilenreihenfolge abhaengt.
  ...
  if (userIds.length) {
    ...
    const nummern = {};
    const pids = [...new Set(alle.map((z) => z.projekt_id).filter(Boolean))];
    if (pids.length) {
      const pr = await sbSoft(`gs_projekte?id=in.(${pids.join(',')})&select=id,projektnummer`, []) || [];
      for (const x of pr) nummern[x.id] = x.projektnummer || '';
    }
  ...
  const tage = [...tageMap.keys()].sort().map((datum) => {
    ...
    const out = rows.map((z) => {
      const abw = z.abwesenheit ? abwesenheitLabel(z.abwesenheit) : null;
      return {
        id: z.id,
    ...
    return {
      ...
      zeilen: out,
      stunden: s('stunden'), uz25: s('uz25'), uz50: s('uz50'), uz100: s('uz100'),
      // Spesen je Kalendertag EINMAL, auch wenn der Tag hier mehrere Zeilen
      // traegt. Zusaetzlich greift die Zurechnung weiter unten: dieser Bericht
      // weist die Tagespauschale nur aus, wenn DIESES Projekt an dem Tag das
      // fuehrende war — sonst zaehlten vier Projektberichte denselben Tag
      // viermal.
      spesen: traegtSpesen(datum) ? Math.max(0, ...out.map((x) => num(x.spesen)), 0) : 0,
      fotos: s('fotos'),
    };
  });
  ...
  if (!fotos.length && !fotosOhneTag.length && !fotosOhneZuordnung.anzahl) hinweise.push('Für diesen Zeitraum liegen keine Fotos vor.');
  // Die Zahl gehoert auch in die Hinweisliste: wer nur die Zusammenfassung
  // liest, soll nicht glauben, es gebe keine Bilder.
  if (fotosOhneZuordnung.anzahl) hinweise.push(ohneZuordnungText(fotosOhneZuordnung));
  ...
  return {
    ...
    fotos_ohne_zuordnung: fotosOhneZuordnung,
...
function leeresErgebnis({ quelle, jahr, woche, von, bis, zielId, hinweise }) {
  return {
    ...
    kopf: {
      ...
      kunde_email: null, partner_email: null,
      partner_id: null,
      jahr, woche, von, bis,
    },
    tage: [],
    summen: { stunden: 0, uz25: 0, uz50: 0, uz100: 0, spesen: 0, tage: 0, zeilen: 0, techniker: 0 },
    material: [], fotos: [], fotos_ohne_tag: [],
    fotos_ohne_zuordnung: { anzahl: 0, grund: null, wochen: [] },
    fotos_vorhanden: false, material_vorhanden: false,
...
// Videos: eigenes, kleines Budget. Abgebildet wird nur das Standbild — der
// Inhalt liegt hinter dem Verweis, nicht auf dem Papier.
const VIDEOS_IM_PDF = 4;
...
export function arbeitenText(z) {
  const teile = [];
...
export function buildWochenberichtPdf(daten, { logo, fotos, fotosOhneTag, videos, berichtNr, branding } = {}) {
  const k = daten.kopf;
  const blocks = [];
  ...
  // Fotos, die wegen der Wochenregel in KEINEM Bericht erscheinen. Alte,
  // eingefrorene Snapshots kennen das Feld nicht — dann ist die Zahl 0 und der
  // Abschnitt entfaellt, das Dokument bleibt unveraendert.
  const ohneZuordnung = daten.fotos_ohne_zuordnung || { anzahl: 0 };
  ...
  if (bilder.length) {
    ...
    blocks.push({
      t: 'text',
      text: ohneZuordnung.anzahl
        ? 'Dieser Woche ist kein Foto zugeordnet — siehe „Fotos ohne Tageszuordnung".'
        : 'Für diesen Zeitraum liegen keine Fotos vor.',
      size: 9, lead: 13,
    });
  } else {
  ...
  // ── Videos (Phase 8) ──
  // Je Video das Standbild, und das Standbild IST der Verweis: ein Tipp im
  // PDF fuehrt auf /v/<token>. Der Token gilt 30 Tage und nur fuer dieses
  // eine Video (lib/videotoken.js) — der Bauleiter braucht keinen Zugang.
  const videoBilder = (videos || []).filter((v) => v && v.buf);
  const videoOhneBild = (daten.videos || []).length - videoBilder.length;
  if (videoBilder.length) {
    blocks.push({ t: 'sp', size: 8 });
    blocks.push({ t: 'need', h: 140 });
    blocks.push({ t: 'h2', text: 'Videos' });
    blocks.push({
      t: 'text', size: 8.5, lead: 13,
      text: `${(daten.videos || []).length} Video(s) dieser Woche. Das Standbild antippen öffnet das Video im Browser; `
        + 'der Link gilt 30 Tage und braucht keine Anmeldung.',
    });
    blocks.push({
      t: 'imgrow', perRow: 2, maxH: 168, gap: 14,
      images: videoBilder.map((v) => v.buf),
      captions: videoBilder.map((v) => v.caption || ''),
      links: videoBilder.map((v) => v.url || null),
    });
  }
  if (videoOhneBild > 0) {
    ...
    blocks.push({
      t: 'text', size: 8.5, lead: 13,
      text: `${videoOhneBild} Video(s) ohne Standbild — sie sind erfasst, lassen sich hier aber nicht abbilden. `
        + `Zu finden im Cockpit: Projekt „${k.titel || ''}" → Medien.`,
    });
  }

  // ── Fotos, die in KEINE Woche fallen — die Zahl steht im Dokument ──
  // Ohne diesen Abschnitt waere die Wochenregel eine stille Loeschung: der
  // Bauleiter saehe weniger Bilder und wuesste nicht, dass es mehr gibt.
  if (ohneZuordnung.anzahl) {
    blocks.push({ t: 'sp', size: 8 });
    blocks.push({ t: 'need', h: 60 });
    blocks.push({ t: 'h2', text: 'Fotos ohne Tageszuordnung' });
    blocks.push({ t: 'text', text: ohneZuordnungText(ohneZuordnung), size: 8.5, lead: 13 });
  }

  // ── Hinweise: alles, was der Bericht NICHT weiss, steht drin ──
  // Der Satz zur fehlenden Tageszuordnung hat oben einen eigenen Abschnitt —
  // hier stuende er ein zweites Mal.
  const ozSatz = ohneZuordnungText(ohneZuordnung);
  const relevant = (daten.hinweise || []).filter((x) => !/keine Fotos/.test(x) && x !== ozSatz);
  if (relevant.length) {
    blocks.push({ t: 'need', h: 60 });
...
export async function sammleWochenrapport({ wochenrapportId }) {
  ...
  // Abwesenheitszeilen laufen GETRENNT. Sie stehen in derselben Spalte
  // (gesamtstunden), sind aber keine geleistete Arbeit — eine Woche mit drei
  // Krankheitstagen darf im Total nicht als voll gearbeitet erscheinen.
  // Überzeit und Spesen gibt es auf einer Abwesenheitszeile ohnehin nicht;
  // der Filter steht trotzdem auf allen, damit hier keine Ausnahme wächst.
  const arbeitsZeilen = zeilen.filter((z) => !z.abwesenheit);
  const r2s = (k) => r2(arbeitsZeilen.reduce((a, z) => a + num(z[k]), 0));
  return {
    kopf: {
    ...
    summen: {
      ...
      uz100: r2s('ueberzeit_100'),
      // NICHT r2s('spesen'): das addierte 8 Zeilen x CHF 30 = 240.00 fuer eine
      // Woche mit 5 Arbeitstagen. Spesen fallen je Kalendertag einmal an.
      spesen: spesenJeTagAus(arbeitsZeilen),
      tage: new Set(arbeitsZeilen.filter((z) => num(z.gesamtstunden) > 0).map((z) => z.datum)).size,
      // Eigene Summe, eigener Block im PDF — nie in `stunden` eingerechnet.
      abwesenheit_stunden: trenneStunden(zeilen).abwesenheit_stunden,
      abwesenheit_tage: new Set(zeilen.filter((z) => z.abwesenheit).map((z) => z.datum)).size,
    },
    abwesenheiten: abwesenheitBloecke(zeilen),
  };
}
...
export function buildWochenrapportPdf(daten, { logo, unterschriften, branding } = {}) {
  ...
  blocks.push({
    ...
    // Abwesenheitszeilen stehen NICHT in der Stundentabelle, sondern in einem
    // eigenen Block darunter. Vorher liefen sie mit '—' in Zeit/Spesen mit und
    // ihre Stunden verschwanden in derselben Summe wie geleistete Arbeit.
    rows: (daten.zeilen || []).filter((z) => !z.abwesenheit).map((z) => {
      ...
      const volltext = z.abwesenheit
        ? `Abwesend: ${abwesenheitLabel(z.abwesenheit)}${z.abwesenheit_grund ? ' (' + z.abwesenheit_grund + ')' : ''}`
  ...
  // ── Abwesenheiten: eigener Block, eigene Summe ──
  // Getrennt ausgewiesen, nicht weggelassen: eine Woche mit zwei Krankheitstagen
  // soll man dem Blatt ansehen, ohne dass die Stunden im Arbeitstotal landen.
  const abw = daten.abwesenheiten || [];
  const abwStunden = (daten.summen && daten.summen.abwesenheit_stunden) || 0;
  if (abw.length) {
    blocks.push({ t: 'sp', size: 14 });
    blocks.push({ t: 'need', h: 90 });
    blocks.push({ t: 'h1', text: 'Abwesenheiten' });
    blocks.push({
      t: 'table', size: 8.5, gap: 4,
      cols: [
        { w: 44, label: 'Kürzel' }, { w: 150, label: 'Grund' },
        { w: 50, label: 'Tage', align: 'right' }, { w: 60, label: 'Stunden', align: 'right' },
        { w: 189, label: 'Bemerkung' },
      ],
      rows: abw.map((a) => [
        a.code, a.label,
        { text: String(a.tage), align: 'right' },
        { text: h(a.stunden), align: 'right' },
        a.gruende.join(' · ') || '—',
      ]),
    });
    blocks.push({ t: 'sp', size: 6 });
    blocks.push({
      t: 'text', bold: true, size: 9, lead: 13,
      text: `Total Abwesenheit: ${h(abwStunden)} h an ${daten.summen.abwesenheit_tage} Tag(en). `
        + 'Diese Stunden sind im Stundentotal oben NICHT enthalten.',
    });
  }
...
export function buildFotodokuPdf(daten, { logo, bilderJeTag, bilderOhneTag, berichtNr, branding, teil, vonTeilen } = {}) {
  const k = daten.kopf;
  const nr = berichtNr || berichtNummer({ kuerzel: k.kuerzel, nummer: k.nummer, jahr: k.jahr, woche: k.woche });
  const blocks = [];
  const marke = branding || null;
  ...
  for (const g of bilderJeTag || []) {
    ...
    // Bei mehreren Projekten in der Woche gehoert das Projekt in die
    // Ueberschrift — sonst stehen zwei Baustellen desselben Tages unter
    // derselben Zeile und niemand sieht, welches Bild wohin gehoert.
    blocks.push({ t: 'h2', text: `${g.wochentag}, ${dmy(g.datum)}${g.mehrereProjekte && g.projekt ? ` — ${g.projekt}` : ''}` });
  ...
  // ── Fotos, die in KEINE Woche fallen ──
  // Je Projekt eine Zeile. Steht in JEDEM Teildokument: jedes Teil ist fuer
  // sich ein Dokument, und diese Zahl darf in keinem fehlen.
  const ozListe = (daten.fotos_ohne_zuordnung || []).filter((x) => x && x.anzahl);
  if (ozListe.length) {
    blocks.push({ t: 'sp', size: 8 });
    blocks.push({ t: 'need', h: 80 });
    blocks.push({ t: 'h2', text: 'Fotos ohne Tageszuordnung' });
    for (const x of ozListe) {
      blocks.push({ t: 'text', text: `${x.projekt}: ${ohneZuordnungText(x)}`, size: 8.5, lead: 12 });
    }
  }

  // Der Restverweis darf NICHT ueber das ganze Dokument rechnen, wenn dieses
  // nur ein Teil ist: die uebrigen Fotos fehlen dann nicht, sie stehen in den
  // anderen Teilen. Sonst behauptet Teil 2/3 "6 Fotos nicht abgebildet",
  // obwohl genau diese 6 in Teil 1 und 3 stehen.
  const rest = ((daten.fotos || []).length + (daten.fotos_ohne_tag || []).length) - gesamt;
  if (rest > 0) {
    blocks.push({ t: 'sp', size: 6 });
    blocks.push({
      t: 'text',
      text: (vonTeilen && vonTeilen > 1)
        ? `Weitere ${rest} Foto(s) dieser Woche stehen in den übrigen Teilen dieser Fotodokumentation (${vonTeilen} Teile insgesamt).`
        : `Weitere ${rest} Foto(s) dieser Woche sind hier nicht abgebildet (Deckel ${FOTOS_DOKU_MAX}, oder kein einbettbares JPEG). Sie liegen in der Projektgalerie: ${COCKPIT_URL}`,
      size: 8.5,
    });
  }

  // Zaehlung nur, wenn es mehr als ein Teildokument gibt. Bei einem einzelnen
  // Dokument waere "1/1" nur Rauschen.
  const zaehlung = (vonTeilen && vonTeilen > 1) ? ` ${teil}/${vonTeilen}` : '';
  return buildPdf({
    balance: true,
    branding: marke || undefined,
    style: 'brief',
    title: `Fotodokumentation${zaehlung}`,
    subtitle: kopfzeile(k, nr),
    logo: logo || (marke && marke.logo) || undefined,
    footer: (k.nummer ? `${nr} · Projekt ${k.nummer}` : nr) + (zaehlung ? ` · Teil${zaehlung}` : ''),
    blocks,
  });
}

// ── Aufteilung grosser Fotodokumentationen ────────────────────────────────
// Schnittkante ist IMMER eine Tages- oder Projektgrenze, nie mitten in einem
// Tag: ein halber Tag auf zwei Dokumenten waere schlimmer als ein zu grosses.
// Passt ein einzelner Tag allein nicht ins Budget, bleibt er ungeteilt und das
// Teildokument wird eben groesser — das wird gemeldet, nicht erzwungen.
//
// Budget: 3.0 MB je Teil. Das ist so gewaehlt, dass ein Teil auch als base64
// (Faktor 1.37) unter dem Vercel-Antwortlimit von 4.5 MB bliebe — falls je
// wieder jemand base64 ausliefern will, geht es dann noch.
const FOTODOKU_TEIL_MAX = 3 * 1024 * 1024;

function teileFotodoku(gruppen, budget = FOTODOKU_TEIL_MAX) {
  const gb = (g) => g.bilder.reduce((a, b) => a + b.buf.length, 0);

  // Gruppen des Auffangnetzes gehoeren zu KEINEM Tag — sie duerfen zerlegt
  // werden, ohne die Regel zu verletzen. Ohne das waere der ganze Auffangposten
  // ein einziger unteilbarer Block: KW 29 mit zehn Fotos ergab so ein Teil von
  // 6.72 MB, obwohl das Budget bei 3 MB liegt.
  const zerlegt = [];
  for (const g of gruppen) {
    if (g.art !== 'ohne_tag' || gb(g) <= budget) { zerlegt.push(g); continue; }
    let block = [], bytes = 0;
    for (const b of g.bilder) {
      if (block.length && bytes + b.buf.length > budget) { zerlegt.push({ ...g, bilder: block }); block = []; bytes = 0; }
      block.push(b); bytes += b.buf.length;
    }
    if (block.length) zerlegt.push({ ...g, bilder: block });
  }

  const teile = [];
  let aktuell = [], groesse = 0;
  for (const g of zerlegt) {
    const b = gb(g);
    if (aktuell.length && groesse + b > budget) { teile.push(aktuell); aktuell = []; groesse = 0; }
    aktuell.push(g); groesse += b;
  }
  if (aktuell.length) teile.push(aktuell);
  return teile.length ? teile : [[]];
}

// PDF in den Storage legen und signiert zurueckgeben. Spiegelt den Weg, den
// versendeBericht fuer wochenberichte/ schon geht.
async function legeFotodokuAb(projektId, dateiname, pdf) {
  const pfad = `fotodoku/${projektId}/${String(dateiname).replace(/[^\w.-]+/g, '_')}`;
  ...
  if (!r.ok) return null;
  const sg = await fetch(`${SUPABASE_URL}/storage/v1/object/sign/${MEDIEN_BUCKET}/${pfad}`, {
    method: 'POST', headers: SB, body: JSON.stringify({ expiresIn: 3600 }),
  });
  const d = await sg.json().catch(() => ({}));
  return { pfad, url: d.signedURL ? SUPABASE_URL + '/storage/v1' + d.signedURL : null };
}
...
// ── Fotos einer ganzen Woche, ueber alle Projekte ─────────────────────────
// ZIEL 4: Die Fotodokumentation fragte bisher nach EINEM Projekt. Sie verhaelt
// sich jetzt wie der Wochenbericht-Knopf: ganze Woche, alle Projekte. Die
// Einschraenkung auf ein Projekt oder einzelne Tage bleibt moeglich, ist aber
// ein Zusatz und keine Pflichtfrage.
//
// Bezugsgroesse ist der WOCHENRAPPORT (Techniker x KW) — der Knopf sitzt in
// dessen Liste. Gesammelt wird ueber die Projekte, die dieser Techniker in
// dieser Woche gebucht hat; nicht firmenweit, nicht je Kunde.
//
// Gruppiert wird nach (Tag, Projekt). Das ist zugleich die Schnittkante fuer
// die Aufteilung: nie mitten in einem Tag, nie mitten in einem Projekt.
export async function sammleFotosDerWoche({ wochenrapportId = null, projektId = null, jahr, woche, nurProjekt = null, nurTage = null }) {
  ...
  if (wochenrapportId) {
    zeilen = await sbSoft(
      `gs_tagesrapporte?wochenrapport_id=eq.${wochenrapportId}`
      + '&select=id,datum,projekt_id,arbeiten,besonderheiten&order=datum.asc,created_at.asc', []) || [];
  } else if (projektId) {
    zeilen = await sbSoft(
      `gs_tagesrapporte?projekt_id=eq.${projektId}&datum=gte.${von}&datum=lte.${bis}`
      + '&select=id,datum,projekt_id,arbeiten,besonderheiten&order=datum.asc,created_at.asc', []) || [];
  }
  if (nurProjekt) zeilen = zeilen.filter((z) => z.projekt_id === nurProjekt);
  if (Array.isArray(nurTage) && nurTage.length) zeilen = zeilen.filter((z) => nurTage.includes(z.datum));

  // Die beteiligten Projekte kommen aus den Tageszeilen der Woche — UND aus dem
  // ausdruecklich angefragten Projekt. Ohne den zweiten Teil war ein Projekt
  // ohne Tageszeilen in dieser Woche gar nicht vorhanden, und damit fiel auch
  // sein Auffangnetz weg: die Fotodokumentation kam leer zurueck, obwohl das
  // Projekt Fotos trug (belegt an 60133.00 Nidelbadstrasse, 21 Bilder, KW 34).
  const projIds = [...new Set([
    ...zeilen.map((z) => z.projekt_id),
    nurProjekt || projektId || null,
  ].filter(Boolean))];
  const projMap = {};
  if (projIds.length) {
    const pr = await sbSoft(`gs_projekte?id=in.(${projIds.join(',')})&select=id,name,projektnummer,standort,kunde_id,kuerzel,partner_user_id`, []) || [];
    for (const x of pr) projMap[x.id] = x;
  }

  // Fotos an den Tageszeilen dieser Woche — ueber ALLE beteiligten Projekte.
  const zeilenIds = zeilen.map((z) => z.id).filter(Boolean);
  let fotos = [];
  if (zeilenIds.length) {
    const m = await sbSoft(
      `gs_projekt_medien?tagesrapport_id=in.(${zeilenIds.join(',')})&medientyp=eq.foto`
      + '&select=id,tagesrapport_id,bucket,path,thumbnail_path,dateiname,stockwerk,wohnung,raum,bauabschnitt,notiz,created_at&order=created_at.asc', []) || [];
    const zuZeile = {};
    for (const z of zeilen) zuZeile[z.id] = z;
    fotos = m.map((x) => ({
      id: x.id, bucket: x.bucket || MEDIEN_BUCKET, path: x.path, thumbnail_path: x.thumbnail_path || null, dateiname: x.dateiname || null,
      datum: (zuZeile[x.tagesrapport_id] || {}).datum || null,
      projekt_id: (zuZeile[x.tagesrapport_id] || {}).projekt_id || null,
      ort: [x.stockwerk, x.wohnung, x.raum, x.bauabschnitt].filter(Boolean).join(' · ') || null,
      notiz: x.notiz || null,
    }));
  }

  // ── Auffangnetz je beteiligtem Projekt — Wochenregel wie im Wochenbericht ─
  // Bis hierher stand hier GAR KEIN Filter. Das war der Grund, warum dieselben
  // zehn Bilder in KW 29, 30 und 31 standen: ein Foto ohne Tagesbindung war
  // Auffangposten JEDER Fotodokumentation seines Projekts.
  //
  // Jetzt entscheidet dieselbe Funktion wie im Wochenbericht — fotoWochenurteil
  // ueber wochenMitStunden. Keine zweite Implementierung, keine zweite
  // Wahrheit: melden beide Dokumente unterschiedliche Zahlen, ist das ein
  // Fehler und keine Auslegungssache.
  //
  // nurTage bleibt ein Ausschluss: wer auf einzelne Tage filtert, will genau
  // diese Tage — Fotos ohne Tag gehoeren zu keinem davon.
  let ohneTag = [];
  const ohneZuordnung = [];
  if (projIds.length && !(Array.isArray(nurTage) && nurTage.length)) {
    const m2 = await sbSoft(
      `gs_projekt_medien?projekt_id=in.(${projIds.join(',')})&tagesrapport_id=is.null&medientyp=eq.foto`
      + '&select=id,projekt_id,bucket,path,thumbnail_path,dateiname,stockwerk,wohnung,raum,bauabschnitt,notiz,created_at&order=created_at.asc', null);
    if (m2 === null) throw new Error('Die Fotos ohne Tageszuordnung konnten nicht gelesen werden.');
    const alle = m2.map((x) => ({
      id: x.id, bucket: x.bucket || MEDIEN_BUCKET, path: x.path, thumbnail_path: x.thumbnail_path || null, dateiname: x.dateiname || null,
      datum: null, projekt_id: x.projekt_id || null,
      hochgeladen: x.created_at ? String(x.created_at).slice(0, 10) : null,
      ort: [x.stockwerk, x.wohnung, x.raum, x.bauabschnitt].filter(Boolean).join(' · ') || null,
      notiz: x.notiz || null,
    }));
    const wochenJeProjekt = await wochenMitStunden(projIds);
    for (const pid of projIds) {
      const desProjekts = alle.filter((f) => f.projekt_id === pid);
      if (!desProjekts.length) continue;
      const urteil = fotoWochenurteil({ wochen: wochenJeProjekt.get(pid) || new Set(), jahr, woche });
      if (urteil.zeigen) { ohneTag.push(...desProjekts); continue; }
      // Nicht gezeigt heisst nicht verschwiegen: je Projekt eine gemeldete Zahl.
      const p = projMap[pid] || {};
      ohneZuordnung.push({
        projekt: [p.projektnummer, p.name].filter(Boolean).join(' · ') || 'Projekt',
        anzahl: desProjekts.length, grund: urteil.grund, wochen: urteil.wochen,
      });
    }
  }

  // Kopf: bei genau einem Projekt der Projektkopf, sonst eine Sammelbezeichnung.
  const einziges = projIds.length === 1 ? projMap[projIds[0]] : null;
  let kunde = null;
  if (einziges && einziges.kunde_id) {
    const kd = await sbSoft(`gs_kunden?id=eq.${einziges.kunde_id}&select=firma&limit=1`, []) || [];
    kunde = (kd[0] || {}).firma || null;
  }
  const kopf = {
    quelle: 'woche',
    titel: einziges ? (einziges.name || null) : `${projIds.length} Projekte`,
    nummer: einziges ? (einziges.projektnummer || null) : null,
    kuerzel: einziges ? (einziges.kuerzel || null) : null,
    adresse: einziges ? (einziges.standort || null) : null,
    kunde,
    partner_id: einziges ? (einziges.partner_user_id || null) : null,
    jahr, woche, von, bis,
  };

  // Gruppen: je (Tag, Projekt) eine. Chronologisch, innerhalb des Tages nach
  // Projektnummer — damit die Reihenfolge stabil ist und nicht an der
  // Fotoreihenfolge haengt.
  const gruppen = [];
  const tage = [...new Set(fotos.map((f) => f.datum).filter(Boolean))].sort();
  for (const datum of tage) {
    const desTages = fotos.filter((f) => f.datum === datum);
    const pids = [...new Set(desTages.map((f) => f.projekt_id))]
      .sort((a, b) => String((projMap[a] || {}).projektnummer || '').localeCompare(String((projMap[b] || {}).projektnummer || '')));
    for (const pid of pids) {
      const p = projMap[pid] || {};
      const arbeiten = zeilen.filter((z) => z.datum === datum && z.projekt_id === pid)
        .map((z) => (Array.isArray(z.arbeiten) ? z.arbeiten.filter(Boolean).join(' · ') : '')).filter(Boolean).join(' · ');
      gruppen.push({
        art: 'tag', datum,
        wochentag: WOCHENTAG[new Date(`${datum}T00:00:00Z`).getUTCDay()],
        projekt: [p.projektnummer, p.name].filter(Boolean).join(' · ') || null,
        mehrereProjekte: projIds.length > 1,
        arbeiten: arbeiten || null,
        fotos: desTages.filter((f) => f.projekt_id === pid),
      });
    }
  }
  // Projektliste stabil sortiert (Set-Reihenfolge haengt sonst an der
  // Zeilenreihenfolge und die Filterknoepfe springen).
  const projekte = projIds.map((id) => projMap[id]).filter(Boolean)
    .sort((a, b) => String(a.projektnummer || '').localeCompare(String(b.projektnummer || '')));
  return { kopf, gruppen, ohneTag, ohneZuordnung, projekte, zeilen };
}

// Erzeugt die Fotodokumentation, teilt sie bei Bedarf und legt jedes Teil im
// Storage ab. Zurueck kommen signierte URLs statt base64 — ein PDF mit zehn
// Fotos war 6.72 MB, als base64 8.96 MB, und Vercel deckelt den Antwortkoerper
// bei 4.5 MB.
export async function erzeugeFotodoku({ projektId = null, wochenrapportId = null, jahr, woche, nurProjekt = null, nurTage = null }) {
  const q = await sammleFotosDerWoche({ wochenrapportId, projektId, jahr, woche, nurProjekt, nurTage });
  const nr = berichtNummer({ kuerzel: q.kopf.kuerzel, nummer: q.kopf.nummer, jahr, woche, praefix: 'FD' });

  // Bytes laden, Deckel von vorn vergeben, damit der Wochenanfang vollstaendig
  // ist statt ueberall Luecken.
  // Phase 9 — erst die Auswahl treffen, dann ALLES parallel laden. Vorher lief
  // hier eine Schleife ueber bis zu 60 Bilder, eines nach dem anderen; das war
  // der langsamste Teil des ganzen Dokuments. Ein Pfad wird nur einmal geholt,
  // auch wenn er in zwei Gruppen vorkommt.
  let rest = FOTOS_DOKU_MAX;
  const auswahlen = [];
  for (const g of q.gruppen) {
    if (rest <= 0) break;
    const auswahl = g.fotos.slice(0, rest);
    rest -= auswahl.length;
    auswahlen.push({ gruppe: g, auswahl });
  }
  const ohneTagAuswahl = (q.ohneTag || []).slice(0, Math.max(0, rest));

  const cache = new Map();
  const hol = (f) => {
    const q = fotoQuelle(f);
    const k = `${q.bucket || MEDIEN_BUCKET}/${q.path}`;
    if (!cache.has(k)) cache.set(k, ladeFotoBytes(f));
    return cache.get(k);
  };
  const alleFotos = [...auswahlen.flatMap((x) => x.auswahl), ...ohneTagAuswahl];
  await Promise.all(alleFotos.map((f) => hol(f)));

  const geladen = [];
  for (const { gruppe, auswahl } of auswahlen) {
    const bilder = [];
    for (const f of auswahl) { const buf = await hol(f); if (buf) bilder.push({ buf, caption: fotoCaption(f) }); }
    if (bilder.length) geladen.push({ ...gruppe, bilder });
  }
  const bilderOhneTag = [];
  for (const f of ohneTagAuswahl) {
    const buf = await hol(f);
    if (buf) bilderOhneTag.push({ buf, caption: fotoCaption(f) });
  }
  if (bilderOhneTag.length) geladen.push({ art: 'ohne_tag', bilder: bilderOhneTag });

  const erfasst = q.gruppen.reduce((a, g) => a + g.fotos.length, 0) + (q.ohneTag || []).length;
  const abgebildet = geladen.reduce((a, g) => a + g.bilder.length, 0);
  // Was die Wochenregel aussortiert hat — gemeldet, nicht weggelassen.
  const ohneZuordnung = q.ohneZuordnung || [];
  const ohneZuordnungAnzahl = ohneZuordnung.reduce((a, x) => a + x.anzahl, 0);

  const teile = teileFotodoku(geladen);
  const marke = await ladeBranding({ partnerId: q.kopf.partner_id || null });
  const logo = marke.logo || await ladeLogo();

  const dokumente = [];
  for (let i = 0; i < teile.length; i++) {
    const grp = teile[i];
    const pdf = buildFotodokuPdf(
      { kopf: q.kopf, fotos: q.gruppen.flatMap((g) => g.fotos), fotos_ohne_tag: q.ohneTag, fotos_ohne_zuordnung: ohneZuordnung },
      {
        logo,
        bilderJeTag: grp.filter((g) => g.art === 'tag'),
        bilderOhneTag: (grp.find((g) => g.art === 'ohne_tag') || {}).bilder || [],
        berichtNr: nr, branding: marke,
        teil: i + 1, vonTeilen: teile.length,
      },
    );
    const name = teile.length > 1 ? `Fotodokumentation_${nr}_${i + 1}von${teile.length}.pdf` : `Fotodokumentation_${nr}.pdf`;
    const ab = await legeFotodokuAb(projektId || (q.projekte[0] || {}).id || 'sammel', name, pdf);
    dokumente.push({
      teil: i + 1, von: teile.length, filename: name,
      bytes: pdf.length,
      seiten: (pdf.toString('latin1').match(/\/Type\s*\/Page[^s]/g) || []).length,
      bilder: grp.reduce((a, g) => a + g.bilder.length, 0),
      url: ab && ab.url, pfad: ab && ab.pfad,
      pdf,                                    // fuer Tests/Abnahme, nicht fuer die API
    });
  }
  return {
    dokumente, kopf: q.kopf, nr, erfasst, abgebildet,
    ohne_zuordnung: { anzahl: ohneZuordnungAnzahl, projekte: ohneZuordnung },
    projekte: q.projekte, deckel: FOTOS_DOKU_MAX,
  };
}
...
export async function fotodokuVorschau({ projektId = null, wochenrapportId = null, jahr, woche, nurProjekt = null, nurTage = null }) {
  const q = await sammleFotosDerWoche({ wochenrapportId, projektId, jahr, woche, nurProjekt, nurTage });
  let rest = FOTOS_DOKU_MAX;
  const gruppen = [];
  for (const g of q.gruppen) {
    const drin = Math.max(0, Math.min(g.fotos.length, rest));
    rest -= drin;
    gruppen.push({
      datum: g.datum, wochentag: g.wochentag, projekt: g.projekt,
      anzahl: g.fotos.length, abgebildet: drin,
    });
  }
  const ohne = q.ohneTag || [];
  const ohneDrin = Math.max(0, Math.min(ohne.length, rest));
  const ozListe = q.ohneZuordnung || [];
  // Geschaetzte Teilzahl: die echte haengt an den Bytes, die hier bewusst nicht
  // geladen werden (die Vorschau soll schnell sein). Gerechnet wird mit dem
  // Mittel der bekannten Dateigroessen, sonst mit einem Erfahrungswert.
  return {
    kopf: q.kopf,
    projekte: q.projekte.map((p) => ({ id: p.id, projektnummer: p.projektnummer, name: p.name })),
    tage: gruppen,
    ohne_tag: { anzahl: ohne.length, abgebildet: ohneDrin, dateien: ohne.map((f) => ({ dateiname: f.dateiname, hochgeladen: f.hochgeladen })) },
    // Die Vorschau zeigt auch, was NICHT kommt — sonst wirkt eine leere Woche
    // wie "keine Fotos vorhanden".
    ohne_zuordnung: {
      anzahl: ozListe.reduce((a, x) => a + x.anzahl, 0),
      projekte: ozListe.map((x) => ({ projekt: x.projekt, anzahl: x.anzahl, grund: x.grund, text: ohneZuordnungText(x) })),
    },
    gesamt: q.gruppen.reduce((a, g) => a + g.fotos.length, 0) + ohne.length,
...
export function fotoCaption(f) {
  ...
  // Ein vom Master vergebener Anzeigename steht VORN — dafuer hat er ihn
  // vergeben. Der automatische Name aus dem Speicherpfad ('IMG_7233.jpeg')
  // erscheint dagegen nicht: er sagt einem Kunden nichts und wuerde jede
  // Bildunterschrift verstopfen. Unterschieden wird beides am Vergleich mit dem
  // Pfad — solange dateiname und Dateiname im Pfad uebereinstimmen, wurde nie
  // umbenannt.
  const name = benannt(f) ? f.dateiname : '';
  return [name, tag, f.ort || f.notiz || ''].filter(Boolean).join(' · ');
}

// Bildunterschrift eines Videos. Wie bei den Fotos, plus Dauer — die ist bei
// einem Video die wichtigste Zusatzangabe (lohnt sich das Antippen?).
export function videoCaption(v) {
  const tag = v.datum ? dmy(v.datum) : '';
  const dauer = v.dauer_sekunden ? `${Math.round(v.dauer_sekunden)} s` : '';
  const name = benannt(v) ? v.dateiname : '';
  return [name, tag, dauer, v.ort || v.notiz || ''].filter(Boolean).join(' · ');
}

// Wurde dieses Foto vom Master umbenannt? gs_projekt_medien.dateiname wird beim
// Hochladen mit dem bereinigten Originalnamen vorbelegt; der Pfad traegt
// denselben Namen mit vorangestelltem Zeitstempel.
function benannt(f) {
  const dn = String(f && f.dateiname || '').trim();
  if (!dn) return false;
  const ausPfad = String(f.path || '').split('/').pop().replace(/^\d{10,}-/, '');
  return dn !== ausPfad;
}
...
export async function erzeugeBericht({ projektId, jahr, woche, userId, einfrieren = false }) {
  ...
  // ── Phase 9: alle Medien PARALLEL laden ────────────────────────────────
  // Vorher lief eine Schleife: vierzehn Anfragen nacheinander, jede mit voller
  // Netzlaufzeit. Der Server wartet dabei die meiste Zeit. Sie haengen nicht
  // voneinander ab — also gehen sie zusammen raus.
  //
  // Ein Bild, das mehrfach vorkommt, wird nur EINMAL geholt: derselbe Pfad
  // liefert dieselben Bytes. lib/pdf.js legt es danach auch nur einmal ins
  // Dokument (imgRef dedupliziert ueber Buffer-Gleichheit).
  const holCache = new Map();
  const hol = (f) => {
    // Schluessel ist die Datei, die WIRKLICH geladen wird (kleine Fassung, sonst
    // Original) — sonst holt derselbe Pfad ueber zwei Eintraege zweimal.
    const q = fotoQuelle(f);
    const schluessel = `${q.bucket || MEDIEN_BUCKET}/${q.path}`;
    if (!holCache.has(schluessel)) holCache.set(schluessel, ladeFotoBytes(f));
    return holCache.get(schluessel);
  };
  ...
  // Videos: das Standbild laden, den Verweis erzeugen. Ein Video OHNE
  // Standbild wird nicht abgebildet — es wird gezaehlt und benannt.
  const videoAuswahl = (daten.videos || []).slice(0, VIDEOS_IM_PDF).filter((v) => v.thumbnail_path);

  const [bytesFotos, bytesOhneTag, bytesVideos] = await Promise.all([
    Promise.all(auswahl.map((f) => hol(f))),
    Promise.all(auswahlOhneTag.map((f) => hol(f))),
    Promise.all(videoAuswahl.map((v) => hol({ bucket: v.bucket, path: v.thumbnail_path }))),
  ]);

  const bilder = [];
  auswahl.forEach((f, i) => { if (bytesFotos[i]) bilder.push({ buf: bytesFotos[i], caption: fotoCaption(f) }); });
  const bilderOhneTag = [];
  auswahlOhneTag.forEach((f, i) => { if (bytesOhneTag[i]) bilderOhneTag.push({ buf: bytesOhneTag[i], caption: fotoCaption(f) }); });
  const videoBilder = [];
  videoAuswahl.forEach((v, i) => {
    if (bytesVideos[i]) videoBilder.push({ buf: bytesVideos[i], caption: videoCaption(v), url: videoLink(v.id) });
  });
  ...
  const pdf = buildWochenberichtPdf(daten, {
    ...
    fotos: bilder, fotosOhneTag: bilderOhneTag, videos: videoBilder, berichtNr: nr, branding: marke,
  });
...
// hinterlegt → gs_projekte.ansprech_email → gs_kunden.email → gs_partner_profil.email.
//
// Die beiden letzten Stufen sind der Grund, warum ueberhaupt je eine Adresse
// vorbelegt ist: live haben 14 von 15 Projekten kein ansprech_email, waehrend
// im Partnerprofil eine gepflegte Adresse steht. Ohne sie muesste der Master
// bei jedem Bericht von Hand tippen — und tat es nicht, weshalb 8 von 9
// Berichten als Entwurf mit leerem Empfaenger liegen blieben.
//
...
//
// Rein: kein DB-Zugriff. Alles, was die Funktion braucht, steht im Kopf — so
// bleibt sie testbar und Anzeige und Versand koennen dieselbe Kette benutzen.
export function empfaengerFuer({ angefragt, kopfRow, daten }) {
  ...
  const kopf = (daten && daten.kopf) || {};

  // Hat der Aufrufer ueberhaupt etwas eingetippt? Dann gilt seine Eingabe —
  // auch wenn sie unbrauchbar ist. Ein Tippfehler darf NICHT still auf die
  // hinterlegte Adresse durchfallen: sonst tippt der Master "haag@nievergelt"
  // (ohne .ch), sieht "versendet" und die Mail ging an jemand anderen als
  // gemeint. Solange es nur eine Rueckfallebene gab, war das kaum sichtbar; mit
  // Kunde und Partnerprofil dahinter waere es eine echte Falle.
  const etwasEingetippt = Array.isArray(angefragt)
    ? angefragt.some((x) => String(x || '').trim())
    : !!String(angefragt || '').trim();
  if (etwasEingetippt) {
    const liste = norm(angefragt);
    return liste.length
      ? { liste, herkunft: 'angefragt' }
      : { liste: [], herkunft: null, ungueltig: true };
  }

  const stufen = [
    ['berichtskopf', kopfRow && kopfRow.empfaenger],
    ['projekt-ansprechperson', kopf.ansprech_email],
    ['kunde', kopf.kunde_email],
    ['partner-profil', kopf.partner_email],
  ];
  for (const [herkunft, wert] of stufen) {
    const liste = norm(wert);
    if (liste.length) return { liste, herkunft };
  }
  return { liste: [], herkunft: null };
}

// Klartext fuer die Oberflaeche: woher der vorbelegte Empfaenger stammt. Der
// Master muss vor dem Senden sehen koennen, wem er da schreibt.
export const EMPFAENGER_HERKUNFT_TEXT = {
  angefragt: 'von Hand eingetragen',
  berichtskopf: 'aus dem Berichtskopf',
  'projekt-ansprechperson': 'aus der Projekt-Ansprechperson',
  kunde: 'aus dem Kundenstamm',
  'partner-profil': 'aus dem Partnerprofil',
};
...
export async function protokolliere({ kopfRow, eintrag, patch }) {
  const mitProtokoll = {
    ...patch,
...
export async function versendeBericht({
  ...
  const { liste, herkunft, ungueltig } = empfaengerFuer({ angefragt: empfaenger, kopfRow: bericht, daten });
  if (!liste.length) {
    return {
  ...
  const eintrag = {
    ...
    // Berichtstyp: unterscheidet den Einzelversand vom Sammelversand
    // (lib/sammelbericht.js schreibt 'sammelbericht'). Reines JSONB-Feld,
    // keine Spalte — Altentraege ohne `typ` gelten als 'wochenbericht'.
    typ: 'wochenbericht',
    bericht_nr: nr,
    von: userId || null,
    ok: erfolg,
...
// ═══════════════════════════════════════════════════════════════════════════
// PHASE 9 — Bilder in der Groesse laden, in der sie gebraucht werden
// ═══════════════════════════════════════════════════════════════════════════
// Bisher wanderte das Original in das PDF: 1602x2400 px und 458 KB fuer ein
// Bild, das im Dokument hoechstens 228 x 168 Punkte gross ist. Bei zehn Fotos
// sind das ueber 6 MB, die niemand sieht — und die auf der Baustelle jedes Mal
// durch das Mobilfunknetz muessen.
//
// GERECHNET, nicht geraten: die Fotoreihe ist zweispaltig, Satzbreite 471 pt,
// eine Spalte also 228.5 pt breit, die Hoehe hoechstens 168 pt. Bei 300 dpi
// (Druck) entspricht das 952 x 700 Pixeln. Alles darueber ist Ballast.
//
// ── Was NICHT geht, und warum es hier steht ──
// Supabase kann Bilder beim Ausliefern umrechnen
// (/storage/v1/render/image/...?width=…). Am 03.09.2026 gegen die Live-Instanz
// geprueft: die Umrechnung funktioniert und liefert 458 KB → 76 KB. Sie gibt
// die Bilder aber als PROGRESSIVE JPEGs zurueck (SOF2), und die kann ein PDF
// nicht einbetten — /DCTDecode verlangt Baseline. lib/pdf.js weist sie
// korrekt ab (imgRef → null), das Ergebnis waere ein Bericht GANZ OHNE Fotos.
// Kein Parameter aendert das (quality, format=origin, progressive=false
// wurden durchprobiert). Deshalb wird dieser Weg NICHT benutzt.
//
// Der Weg, der bleibt: die kleine Fassung entsteht dort, wo es einen Encoder
// gibt — auf dem Geraet beim Hochladen (app.html, canvas.toDataURL liefert
// Baseline). Sie liegt in gs_projekt_medien.thumbnail_path, und der Bericht
// nimmt sie, wenn es sie gibt. Fuer Altbestand ohne kleine Fassung bleibt das
// Original; verloren geht nichts.
export function fotoQuelle(foto) {
  if (!foto) return foto;
  // Die kleine Fassung ist fuer die Abbildung gemacht. Das Original bleibt im
  // Bucket und ist ueber die Galerie weiterhin in voller Groesse da.
  return foto.thumbnail_path ? { bucket: foto.bucket, path: foto.thumbnail_path } : foto;
}

export async function ladeFotoBytes(foto, maxBytes = 3 * 1024 * 1024) {
  const q = fotoQuelle(foto);
  if (!q || !q.path) return null;
  const bucket = q.bucket || MEDIEN_BUCKET;
  try {
    const r = await fetch(`${SUPABASE_URL}/storage/v1/object/${bucket}/${q.path}`, { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } });
    if (!r.ok) return null;
