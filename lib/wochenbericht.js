...
export async function sammleFotosDerWoche({ wochenrapportId = null, projektId = null, jahr, woche, nurProjekt = null, nurTage = null }) {
  ...
  // ── Auffangnetz je beteiligtem Projekt — OHNE Zeitfenster ────────────────
  // Bis hierher war dieser Block zusaetzlich auf created_at BETWEEN von..bis
  // eingegrenzt. Das hat ihn in genau dem Fall ausgehebelt, fuer den er gebaut
  // wurde: 21 Fotos des Projekts 60133.00 wurden am 24.08. hochgeladen, die
  // dokumentierte Woche KW 34 endet am 23.08. — die Fotodokumentation kam mit
  // null Bildern zurueck. Ein Foto ohne tagesrapport_id hat keinen Tag; sein
  // HOCHLADE-Zeitpunkt ist kein Aufnahmetag und taugt nicht als Wochenfilter.
  //
  // Preis dieser Entscheidung, bewusst in Kauf genommen: solange ein Foto keine
  // Tageszuordnung hat, erscheint es im Auffangposten JEDER Fotodokumentation
  // dieses Projekts. Sobald es einer Tageszeile zugeordnet ist, faellt es aus
  // dem Auffangnetz heraus und steht nur noch bei seinem Tag. Der Wochenbericht
  // (sammleWochendaten) behaelt sein Zeitfenster — er geht als Wochenleistung
  // an den Kunden, dort waeren Wiederholungen schaedlich.
  //
  ...
  if (projIds.length && !(Array.isArray(nurTage) && nurTage.length)) {
    const m2 = await sbSoft(
      ...
      + '&select=id,projekt_id,bucket,path,dateiname,stockwerk,wohnung,raum,bauabschnitt,notiz,created_at&order=created_at.asc', []) || [];
    ohneTag = m2.map((x) => ({
      id: x.id, bucket: x.bucket || MEDIEN_BUCKET, path: x.path, dateiname: x.dateiname || null,
  ...
  return { kopf, gruppen, ohneTag, projekte, zeilen };
}
...
export async function erzeugeFotodoku({ projektId = null, wochenrapportId = null, jahr, woche, nurProjekt = null, nurTage = null }) {
  ...
  for (let i = 0; i < teile.length; i++) {
    ...
    const pdf = buildFotodokuPdf(
      { kopf: q.kopf, fotos: q.gruppen.flatMap((g) => g.fotos), fotos_ohne_tag: q.ohneTag },
      {
        logo,
  ...
  return { dokumente, kopf: q.kopf, nr, erfasst, abgebildet, projekte: q.projekte, deckel: FOTOS_DOKU_MAX };
}
