...
export function buildWochenberichtPdf(daten, { logo, fotos, berichtNr, branding } = {}) {
  const k = daten.kopf;
  const blocks = [];
...
export async function erzeugeBericht({ projektId, jahr, woche, userId, einfrieren = false }) {
  ...
  const pdf = buildWochenberichtPdf(daten, {
    ...
    fotos: bilder, berichtNr: nr, branding: marke,
  });
  ...
  return {
    ...
    aus_snapshot: ausSnapshot, fotos_im_pdf: bilder.length,
