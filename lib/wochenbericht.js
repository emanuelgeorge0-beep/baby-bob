...
export async function sammleFotosDerWoche({ wochenrapportId = null, projektId = null, jahr, woche, nurProjekt = null, nurTage = null }) {
  ...
  if (zeilenIds.length) {
    const m = await sbSoft(
      `gs_projekt_medien?tagesrapport_id=in.(${zeilenIds.join(',')})&medientyp=eq.foto`
      + '&select=id,tagesrapport_id,bucket,path,dateiname,stockwerk,wohnung,raum,bauabschnitt,notiz,created_at&order=created_at.asc', []) || [];
    const zuZeile = {};
    for (const z of zeilen) zuZeile[z.id] = z;
    fotos = m.map((x) => ({
      id: x.id, bucket: x.bucket || MEDIEN_BUCKET, path: x.path, dateiname: x.dateiname || null,
  ...
  if (projIds.length && !(Array.isArray(nurTage) && nurTage.length)) {
    const m2 = await sbSoft(
      ...
      + '&select=id,projekt_id,bucket,path,dateiname,stockwerk,wohnung,raum,bauabschnitt,notiz,created_at&order=created_at.asc', null);
...
export async function erzeugeFotodoku({ projektId = null, wochenrapportId = null, jahr, woche, nurProjekt = null, nurTage = null }) {
  ...
  for (const g of q.gruppen) {
    ...
    const bilder = [];
    for (const f of auswahl) { const buf = await ladeFotoBytes(f); if (buf) bilder.push({ buf, caption: fotoCaption(f) }); }
    rest -= auswahl.length;
    if (bilder.length) geladen.push({ ...g, bilder });
  }
  const bilderOhneTag = [];
  for (const f of (q.ohneTag || []).slice(0, Math.max(0, rest))) {
...
export async function erzeugeBericht({ projektId, jahr, woche, userId, einfrieren = false }) {
  ...
  const videoBilder = [];
  for (const v of (daten.videos || []).slice(0, VIDEOS_IM_PDF)) {
    if (!v.thumbnail_path) continue;
    const buf = await ladeFotoBytes({ bucket: v.bucket, path: v.thumbnail_path });
    if (!buf) continue;
    videoBilder.push({ buf, caption: videoCaption(v), url: videoLink(v.id) });
  }
