...
export function buildWochenberichtPdf(daten, { logo, fotos, berichtNr } = {}) {
  ...
  if (alleNamen.length) {
    blocks.push({ t: 'kv', label: 'Techniker', value: alleNamen.join(' · ') });
  }
  ...
  for (const tag of aktiveTage) {
    ...
    const cols = [];
    if (!einTechniker) cols.push({ w: 100, label: 'Techniker' });
    cols.push({ w: 62, label: 'Zeit' }, { w: 34, label: 'Std', align: 'right' },
      { w: 34, label: 'ÜZ', align: 'right' }, { w: 55, label: 'Gewerk' },
      { w: einTechniker ? 298 : 198, label: 'Ausgeführte Arbeiten' });
    blocks.push({
