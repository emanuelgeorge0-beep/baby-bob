...
// Einladungs-Mail für neue Techniker: Zugangslink zum Passwort-Setzen (Magic-/Recovery-Link),
// mit Temp-Passwort + Login-URL als Fallback. Look = GS-Hülle (mailShell).
export function technikerInviteHtml({ name, setLink, tempPassword, loginUrl }) {
  const inner = `
    <div style="display:inline-block;background:#FFD24A;color:#0a1628;font-weight:800;font-size:12px;padding:4px 12px;border-radius:50px;margin-bottom:14px;">👷 TECHNIKER-ZUGANG</div>
    <h2 style="margin:0 0 6px;font-size:20px;color:#fff;">Willkommen im Team${name ? ', ' + mailEsc(name) : ''}!</h2>
    <p style="margin:0 0 18px;color:rgba(232,237,245,0.75);">Dein Zugang zum George&nbsp;Solutions Techniker-Cockpit ist bereit. Klick auf den Button, um dein Passwort zu setzen und loszulegen.</p>
    ${setLink ? `<a href="${mailEsc(setLink)}" style="display:inline-block;background:#FFD24A;color:#0a1628;font-weight:800;text-decoration:none;padding:13px 26px;border-radius:10px;font-size:15px;">Passwort setzen &amp; einloggen →</a>` : ''}
    ${tempPassword ? `<div style="margin-top:18px;padding:12px 14px;background:rgba(255,255,255,0.04);border-radius:10px;">
      <div style="color:rgba(232,237,245,0.5);font-size:12px;margin-bottom:6px;">Falls der Button nicht funktioniert: bei <a href="${mailEsc(loginUrl || '')}" style="color:#4A9EFF;text-decoration:none;">${mailEsc((loginUrl || '').replace(/^https?:\/\//, ''))}</a> mit deiner E-Mail und diesem Start-Passwort einloggen:</div>
      <div style="color:#fff;font-family:monospace;font-size:17px;font-weight:700;letter-spacing:1px;">${mailEsc(tempPassword)}</div>
      <div style="color:rgba(232,237,245,0.45);font-size:12px;margin-top:6px;">Beim ersten Login wählst du ein eigenes Passwort.</div>
    </div>` : ''}
  `;
  return mailShell(inner);
}
...
// ═══════════════════════════════════════════════════════════════════════════
// Wochenbericht (Projekt × KW) — Begleitmail an den Bauleiter.
// ═══════════════════════════════════════════════════════════════════════════
// Die MAIL trägt die GS-Hülle wie jede andere Benachrichtigung hier. Das
// DOKUMENT im Anhang ist nach Eiserner Regel 6 hell, weiss, druckbar — das ist
// der Teil, der beim Empfänger auf Papier landet.
//
// Der Mailtext bleibt bewusst knapp: Zahlen zum Überfliegen, Details im PDF.
// Was der Bericht NICHT weiss (fehlende Fotos, nicht eingereichte Techniker),
// steht mit drin — eine Wochenmeldung, die Lücken verschweigt, ist schlimmer
// als keine.
export function wochenberichtEmailHtml({ berichtNr, kw, jahr, projektName, von, bis, summen, einreichstatus, hinweise }) {
  const s = summen || {};
  const uz = Number(s.uz25 || 0) + Number(s.uz50 || 0) + Number(s.uz100 || 0);
  const offen = (einreichstatus || []).filter((e) => e.status !== 'eingereicht');
  const kachel = (wert, label, gold) => `<td style="padding:10px;background:${gold ? 'rgba(255,210,74,0.1)' : 'rgba(255,255,255,0.04)'};border-radius:8px;text-align:center;">`
    + `<div style="font-size:22px;font-weight:800;color:${gold ? '#FFD24A' : '#fff'};">${mailEsc(String(wert))}</div>`
    + `<div style="font-size:11px;color:rgba(232,237,245,0.5);">${mailEsc(label)}</div></td>`;

  const offenHtml = offen.length
    ? `<div style="margin:16px 0 0;padding:12px 14px;background:rgba(255,210,74,0.08);border-left:3px solid #FFD24A;border-radius:6px;">
         <div style="color:#FFD24A;font-weight:700;font-size:13px;margin-bottom:6px;">Noch nicht eingereicht</div>
         ${offen.map((e) => `<div style="color:rgba(232,237,245,0.75);font-size:13px;">${mailEsc(e.techniker)} — ${mailEsc(e.status)}</div>`).join('')}
       </div>`
    : `<p style="margin:16px 0 0;color:rgba(232,237,245,0.6);font-size:13px;">Alle beteiligten Techniker haben die Woche eingereicht.</p>`;

  const hinweisHtml = (hinweise || []).length
    ? `<div style="margin:14px 0 0;color:rgba(232,237,245,0.55);font-size:12px;line-height:1.5;">
         ${hinweise.map((x) => `• ${mailEsc(x)}`).join('<br>')}
       </div>`
    : '';

  const inner = `
    <div style="display:inline-block;background:#FFD24A;color:#0a1628;font-weight:800;font-size:12px;padding:4px 12px;border-radius:50px;margin-bottom:14px;">WOCHENBERICHT</div>
    <h2 style="margin:0 0 4px;font-size:20px;color:#fff;">KW ${mailEsc(String(kw))}/${mailEsc(String(jahr))}</h2>
    <p style="margin:0 0 4px;color:rgba(232,237,245,0.75);">${mailEsc(projektName || '')}</p>
    <p style="margin:0 0 16px;color:rgba(232,237,245,0.5);font-size:13px;">${mailEsc(von || '')} – ${mailEsc(bis || '')} · ${mailEsc(berichtNr || '')}</p>
    <table style="width:100%;border-collapse:collapse;font-size:14px;">
      <tr>
        ${kachel(Number(s.stunden || 0).toFixed(2), 'Stunden', true)}
        <td style="width:8px;"></td>
        ${kachel(uz.toFixed(2), 'Überzeit')}
        <td style="width:8px;"></td>
        ${kachel(s.tage || 0, 'Tage')}
        <td style="width:8px;"></td>
        ${kachel(s.techniker || 0, 'Techniker')}
      </tr>
    </table>
    ${offenHtml}
    ${hinweisHtml}
    <p style="margin:18px 0 0;color:rgba(232,237,245,0.6);font-size:13px;">Der vollständige Bericht liegt als PDF im Anhang — Tagesverlauf, Tätigkeiten und Fotos.</p>
  `;
  return mailShell(inner);
}

// ═══════════════════════════════════════════════════════════════════════════
// Projekt-Exporte (Materialliste · Arbeitsrapporte · Rechnungs-History)
// ═══════════════════════════════════════════════════════════════════════════
// Eine Huelle fuer alle drei Abschnitte: sie unterscheiden sich nur in Titel
// und Kennzahl. Drei fast gleiche Vorlagen waeren drei Orte, an denen die
// Fusszeile auseinanderlaeuft.
//
// Wie beim Wochenbericht gilt Eiserne Regel 6: die MAIL traegt die dunkle
// GS-Huelle, das DOKUMENT im Anhang bleibt hell, weiss und druckbar. Was beim
// Empfaenger auf Papier landet, ist der Anhang — nie diese Mail.
export function exportEmailHtml({ titel, projektName, projektnummer, anzahl, summe, zusatz }) {
  const kachel = (wert, label, gold) => `<td style="padding:10px;background:${gold ? 'rgba(255,210,74,0.1)' : 'rgba(255,255,255,0.04)'};border-radius:8px;text-align:center;">`
    + `<div style="font-size:22px;font-weight:800;color:${gold ? '#FFD24A' : '#fff'};">${mailEsc(String(wert))}</div>`
    + `<div style="font-size:11px;color:rgba(232,237,245,0.5);">${mailEsc(label)}</div></td>`;

  // Zweite Kachel nur, wenn es etwas zu zeigen gibt: eine Summe von CHF 0.00
  // auf einer Materialliste ohne Preise behauptet mehr, als bekannt ist.
  const kennzahl = (summe != null && Number(summe) > 0)
    ? kachel('CHF ' + Number(summe).toLocaleString('de-CH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }), 'Summe')
    : (zusatz ? kachel(zusatz, 'Total') : '');

  const inner = `
    <div style="display:inline-block;background:#FFD24A;color:#0a1628;font-weight:800;font-size:12px;padding:4px 12px;border-radius:50px;margin-bottom:14px;">${mailEsc(String(titel || 'EXPORT').toUpperCase())}</div>
    <h2 style="margin:0 0 4px;font-size:20px;color:#fff;">${mailEsc(titel || '')}</h2>
    <p style="margin:0 0 4px;color:rgba(232,237,245,0.75);">${mailEsc(projektName || '')}</p>
    <p style="margin:0 0 16px;color:rgba(232,237,245,0.5);font-size:13px;">${mailEsc(projektnummer || 'ohne Projektnummer')}</p>
    <table style="width:100%;border-collapse:collapse;font-size:14px;">
      <tr>
        ${kachel(Number(anzahl || 0), 'Positionen', true)}
        ${kennzahl ? '<td style="width:8px;"></td>' + kennzahl : ''}
      </tr>
    </table>
    <p style="margin:18px 0 0;color:rgba(232,237,245,0.6);font-size:13px;">Die vollständige Aufstellung liegt als PDF im Anhang.</p>
    <p style="margin:10px 0 0;color:rgba(232,237,245,0.45);font-size:12px;">Stand: ${mailEsc(new Date().toISOString().slice(0, 16).replace('T', ' '))} — der Export bildet den Datenbestand zum Zeitpunkt des Versands ab und wird nicht eingefroren.</p>
  `;
  return mailShell(inner);
}
