...
// Zeile erscheint.
const SAMMEL_VIDEOS_JE_PROJEKT = 4;
const SAMMEL_VIDEOS_MAX = 16;

// Lädt die Standbilder für alle Projekte. Rückgabe:
//   { bilder: { [projekt_id]: [{buf, caption, url}] }, erfasst, abgebildet, ohne_standbild }
//
// Reihenfolge der Vergabe: Projekt für Projekt von vorn, wie beim Deckel der
// Fotodokumentation — so ist wenigstens der Anfang vollständig, statt überall
// Lücken zu haben.
export async function ladeSammelVideos(sammel) {
  const auswahlJeProjekt = [];
  let rest = SAMMEL_VIDEOS_MAX, erfasst = 0, ohneStandbild = 0;
  for (const p of sammel.projekte || []) {
    ...
    erfasst += alle.length;
    // Ein Video ohne Standbild lässt sich nicht abbilden. Es wird gezählt,
    // nicht verschwiegen — genauso wie im Einzelbericht.
    const mitBild = alle.filter((v) => v.thumbnail_path);
    ohneStandbild += alle.length - mitBild.length;
    const auswahl = mitBild.slice(0, Math.min(SAMMEL_VIDEOS_JE_PROJEKT, Math.max(0, rest)));
    rest -= auswahl.length;
    if (auswahl.length) auswahlJeProjekt.push({ projekt_id: p.projekt_id, auswahl });
  }

  // Parallel laden, jede Datei nur einmal — dieselbe Regel wie im
  // Einzelbericht (Phase 9).
  const cache = new Map();
  const hol = (v) => {
    const q = fotoQuelle({ bucket: v.bucket, path: v.thumbnail_path });
    const schluessel = `${q.bucket || MEDIEN_BUCKET}/${q.path}`;
    if (!cache.has(schluessel)) cache.set(schluessel, ladeFotoBytes({ bucket: v.bucket, path: v.thumbnail_path }));
    return cache.get(schluessel);
  };
  const flach = auswahlJeProjekt.flatMap((x) => x.auswahl);
  const bytes = await Promise.all(flach.map(hol));
  const zuBytes = new Map();
  flach.forEach((v, i) => zuBytes.set(v, bytes[i]));

  const bilder = {};
  let abgebildet = 0;
  for (const { projekt_id, auswahl } of auswahlJeProjekt) {
    const liste = [];
    for (const v of auswahl) {
      const buf = zuBytes.get(v);
      if (!buf) continue;
      liste.push({ buf, caption: videoCaption(v), url: videoLink(v.id) });
    }
    if (liste.length) { bilder[projekt_id] = liste; abgebildet += liste.length; }
  }
  return { bilder, erfasst, abgebildet, ohne_standbild: ohneStandbild };
}
...
export async function erzeugeSammelbericht({ projektIds, jahr, woche, kuerzel, empfaenger }) {
  ...
  if (nichtGezeigt > 0) {
    sammel.hinweise = [...(sammel.hinweise || []), `${nichtGezeigt} von ${vids.erfasst} Video(s) sind nicht abgebildet`
      + (vids.ohne_standbild ? ` (${vids.ohne_standbild} ohne Standbild)` : '')
